...
public static class Generic
{
    ...
    public static object DynamicAsmInvoke(byte[] asmStub, Type functionDelegateType, ref object[] parameters)
    {
        unsafe
        {
            fixed (byte* buffer = asmStub)
            {
                ...
                var oldProtect = Native.NtProtectVirtualMemory(new IntPtr(-1), ref ptr, ref size, DataWin32.WinNT.PAGE_EXECUTE_READWRITE);

                var result = DynamicFunctionInvoke(ptr, functionDelegateType, ref parameters);

                Native.NtProtectVirtualMemory(new IntPtr(-1), ref ptr, ref size, oldProtect);
    ...
    {
        var uModuleName = new DataNative.UNICODE_STRING();
        ...
        if (callResult != DataNative.NTSTATUS.Success || hModule == IntPtr.Zero)
            return IntPtr.Zero;
    ...
    public static IntPtr GetPebLdrModuleEntry(string dllName)
    {
        ...
        var le = (DataNative.LIST_ENTRY)Marshal.PtrToStructure(pInLoadOrderModuleList, typeof(DataNative.LIST_ENTRY));
    ...
    public static byte[] GetSyscallStub(IntPtr pPeb, string functionName)
    {
        ...
        var pebLdrData = Marshal.ReadIntPtr((IntPtr)((ulong)pPeb + ldrDataOffset));
        var pInLoadOrderModuleList = (IntPtr)((ulong)pebLdrData + inLoadOrderModuleListOffset);
        var le = Marshal.PtrToStructure<DataNative.LIST_ENTRY>(pInLoadOrderModuleList);
    ...
    {
        var aFunc = new DataNative.ANSI_STRING
        {
    ...
    public static void CallMappedPEModule(PE.PE_META_DATA peInfo, IntPtr moduleMemoryBase)
    {
        ...
        Native.NtCreateThreadEx(
            ref hRemoteThread,
            DataWin32.WinNT.ACCESS_MASK.STANDARD_RIGHTS_ALL,
