...
public static class Map
{
    ...
    public static PE.PE_MANUAL_MAP MapModuleFromDiskToSection(string dllPath)
    {
        ...
        var objectName = new DataNative.UNICODE_STRING();
        ...
        var objectAttributes = new DataNative.OBJECT_ATTRIBUTES();
        objectAttributes.Length = Marshal.SizeOf(objectAttributes);
        objectAttributes.ObjectName = pObjectName;
        objectAttributes.Attributes = 0x40;

        var ioStatusBlock = new DataNative.IO_STATUS_BLOCK();

        var hFile = IntPtr.Zero;
            
        Native.NtOpenFile(
            ref hFile,
            DataWin32.Kernel32.FileAccessFlags.FILE_READ_DATA |
            DataWin32.Kernel32.FileAccessFlags.FILE_EXECUTE |
            DataWin32.Kernel32.FileAccessFlags.FILE_READ_ATTRIBUTES |
            DataWin32.Kernel32.FileAccessFlags.SYNCHRONIZE,
            ref objectAttributes, ref ioStatusBlock,
            DataWin32.Kernel32.FileShareFlags.FILE_SHARE_READ |
            DataWin32.Kernel32.FileShareFlags.FILE_SHARE_DELETE,
            DataWin32.Kernel32.FileOpenFlags.FILE_SYNCHRONOUS_IO_NONALERT |
            DataWin32.Kernel32.FileOpenFlags.FILE_NON_DIRECTORY_FILE
        );
        ...
        var ret = Native.NtCreateSection(
            ref hSection,
            (uint)DataWin32.WinNT.ACCESS_MASK.SECTION_ALL_ACCESS,
            IntPtr.Zero,
            ref maxSize,
            DataWin32.WinNT.PAGE_READONLY,
            DataWin32.WinNT.SEC_IMAGE,
            hFile
        );
        ...
        Native.NtMapViewOfSection(
            ...
            ref maxSize, 0x2, 0x0,
            DataWin32.WinNT.PAGE_READWRITE
        );

        var secMapObject = new PE.PE_MANUAL_MAP
        {
    ...
    public static void RelocateModule(PE.PE_META_DATA peMetaData, IntPtr moduleMemoryBase)
    {
        ...
        {
            var ibr = new PE.IMAGE_BASE_RELOCATION();
    ...
    /// <param name="moduleMemoryBase">Base address of the module in memory.</param>
    /// <returns>void</returns>
    public static void RewriteModuleIAT(PE.PE_META_DATA peMetaData, IntPtr moduleMemoryBase)
    {
        ...
        var osVersion = new DataNative.OSVERSIONINFOEX();
        ...
        var counter = 0;
        var iid = new DataWin32.Kernel32.IMAGE_IMPORT_DESCRIPTOR();
        iid = (DataWin32.Kernel32.IMAGE_IMPORT_DESCRIPTOR)Marshal.PtrToStructure(
            (IntPtr)((ulong)pImportTable + (uint)(Marshal.SizeOf(iid) * counter)),
            typeof(DataWin32.Kernel32.IMAGE_IMPORT_DESCRIPTOR)
        );
        ...
        while (iid.Name != 0)
        {
            ...
            {
                var oft_itd = new PE.IMAGE_THUNK_DATA32();
                for (var i = 0;; i++)
                {
                    oft_itd = (PE.IMAGE_THUNK_DATA32)Marshal.PtrToStructure(
                        (IntPtr)((ulong)moduleMemoryBase + iid.OriginalFirstThunk + (uint)(i * sizeof(uint))),
                        typeof(PE.IMAGE_THUNK_DATA32));
            ...
            {
                var oft_itd = new PE.IMAGE_THUNK_DATA64();
                    
                for (var i = 0;; i++)
                {
                    oft_itd = (PE.IMAGE_THUNK_DATA64)Marshal.PtrToStructure(
                        (IntPtr)((ulong)moduleMemoryBase + iid.OriginalFirstThunk + (ulong)(i * sizeof(ulong))),
                        typeof(PE.IMAGE_THUNK_DATA64));
    ...
    /// <param name="moduleMemoryBase">Base address of the module in memory.</param>
    /// <returns>void</returns>
    public static void SetModuleSectionPermissions(PE.PE_META_DATA peMetaData, IntPtr moduleMemoryBase)
    {
        var baseOfCode = peMetaData.Is32Bit ? (IntPtr)peMetaData.OptHeader32.BaseOfCode : (IntPtr)peMetaData.OptHeader64.BaseOfCode;
        Native.NtProtectVirtualMemory((IntPtr)(-1), ref moduleMemoryBase, ref baseOfCode, DataWin32.WinNT.PAGE_READONLY);

        foreach (var ish in peMetaData.Sections)
        {
            var isRead = (ish.Characteristics & PE.DataSectionFlags.MEM_READ) != 0;
            var isWrite = (ish.Characteristics & PE.DataSectionFlags.MEM_WRITE) != 0;
            var isExecute = (ish.Characteristics & PE.DataSectionFlags.MEM_EXECUTE) != 0;
                
            uint flNewProtect = 0;
                
            if (isRead & !isWrite & !isExecute)
                flNewProtect = DataWin32.WinNT.PAGE_READONLY;
            else if (isRead & isWrite & !isExecute)
                flNewProtect = DataWin32.WinNT.PAGE_READWRITE;
            else if (isRead & isWrite & isExecute)
                flNewProtect = DataWin32.WinNT.PAGE_EXECUTE_READWRITE;
            else if (isRead & !isWrite & isExecute)
                flNewProtect = DataWin32.WinNT.PAGE_EXECUTE_READ;
            else if (!isRead & !isWrite & isExecute)
                flNewProtect = DataWin32.WinNT.PAGE_EXECUTE;
            else
    ...
    public static PE.PE_MANUAL_MAP MapModuleToMemory(IntPtr pModule)
    {
        ...
        var pImage = Native.NtAllocateVirtualMemory(
            ...
            DataWin32.Kernel32.MEM_COMMIT | DataWin32.Kernel32.MEM_RESERVE,
            DataWin32.WinNT.PAGE_READWRITE
        );
    ...
    /// <returns>PE_MANUAL_MAP object</returns>
    public static PE.PE_MANUAL_MAP MapModuleToMemory(IntPtr pModule, IntPtr pImage, PE.PE_META_DATA peMetaData)
    {
        if (peMetaData.Is32Bit && IntPtr.Size == 8 || !peMetaData.Is32Bit && IntPtr.Size == 4)
        {
            Marshal.FreeHGlobal(pModule);
    ...
    public static void FreeModule(PE.PE_MANUAL_MAP peManualMap)
    {
        ...
        else
        {
            ...
            Native.NtFreeVirtualMemory((IntPtr)(-1), ref pModule, ref size, DataWin32.Kernel32.MEM_RELEASE);
        }
    }
    ...
    public static IntPtr GetSyscallStub(string functionName)
    {
        ...
        Native.NtProtectVirtualMemory((IntPtr)(-1), ref pCallStub, ref regionSize, DataWin32.WinNT.PAGE_EXECUTE_READ);
            
        Marshal.FreeHGlobal(pModule);
        regionSize = peMetaData.Is32Bit ? (IntPtr)peMetaData.OptHeader32.SizeOfImage : (IntPtr)peMetaData.OptHeader64.SizeOfImage;

        Native.NtFreeVirtualMemory((IntPtr)(-1), ref pImage, ref regionSize, DataWin32.Kernel32.MEM_RELEASE);
